...
bool iVr::OculusInit(void)
{
	...
	//com_engineHz.SetInteger(hmdHz); <-- NOT COMPILING
	common->Printf("Hmd: %s .\n", hmdDesc.ProductName);
...
bool iVr::OpenVRInit(void)
{
	...
	*/
	...
	//com_engineHz.SetInteger(vrSystem->hmdHz); <-- not compiling
...
// my own implementation, free of the poop poop
void iVr::HMDInitializeDistortion()
{
	//ovrSizei recommenedTex0Size = ovr_GetFovTextureSize(hmdSession, ovrEye_Left, hmdDesc.DefaultEyeFov[0], 1.0f);
	//ovrSizei recommenedTex1Size = ovr_GetFovTextureSize(hmdSession, ovrEye_Right, hmdDesc.DefaultEyeFov[1], 1.0f);
	//ovrSizei bufferSize;
	...
	for (int eye = 0; eye < 2; eye++)
	{
		...
		common->Printf("Eye %d Rendertarget Width x Height = %d x %d\n", eye, rendertarget.w, rendertarget.h);
	}
...
//{
//
//	if ((!vrSystem->hmdSession || !vrSystem->hasOculusRift || !vr_enable.GetBool()) &&
...
//
//	if (!renderSystem->GetGlConfig().framebufferObjectAvailable)
//	{
...
//
//
//		//VR_AAmode = r_multiSamples.GetInteger() == 0 ? VR_AA_NONE : VR_AA_MSAA; <-- NOT COMPILING
//		VR_AAmode = VR_AA_NONE;
//
//		//common->Printf("vr_FBOAAmode %d r_multisamples %d\n", VR_AAmode, r_multiSamples.GetInteger()); <-- NOT COMPILING
//
//		/*
...
void iVr::HMDGetOrientation(idAngles &hmdAngles, idVec3 &headPositionDelta, idVec3 &bodyPositionDelta, idVec3 &absolutePosition, float playerHeight, bool resetTrackingOffset)
{
	...
	if (vr_frameCheck.GetInteger() == 1 && idLib::frameNumber == lastFrame) //&& !vrSystem->renderingSplash )
	{
		...
		if (resetTrackingOffset == true)
		{
			...
			if (vr_useFloorHeight.GetInteger() == 0)
				trackingOriginOffset.z += playerHeight + 5 + CM_CLIP_EPSILON - vr_normalViewHeight.GetFloat() / vr_scale.GetFloat();
			else if (vr_useFloorHeight.GetInteger() == 2)
				trackingOriginOffset.z += 5;
			else if (vr_useFloorHeight.GetInteger() == 3)
				trackingOriginOffset.z = playerHeight + 5 + CM_CLIP_EPSILON;
	...
	if (resetTrackingOffset == true)
	{
		...
		else if (vr_useFloorHeight.GetInteger() == 4)
		{
			float oldScale = vr_scale.GetFloat();
			float h = trackingOriginHeight * oldScale;
			float newScale = h / (playerHeight + 5 + CM_CLIP_EPSILON);
			trackingOriginHeight *= oldScale / newScale;
			trackingOriginOffset *= oldScale / newScale;
			vr_scale.SetFloat(newScale);
		}
...
void iVr::HMDResetTrackingOriginOffset(void)
{
	...
	static float playerHeight = 0;

	common->Printf("HMDResetTrackingOriginOffset called\n ");

	HMDGetOrientation(rot, head, body, absPos, playerHeight, true);
...
void iVr::CalcAimMove(float &yawDelta, float &pitchDelta)
{
	...
	if (vrSystem->independentWeaponPitch >= pitchDeadzone) vrSystem->independentWeaponPitch = pitchDeadzone;
	if (vrSystem->independentWeaponPitch < -pitchDeadzone) vrSystem->independentWeaponPitch = -pitchDeadzone;
	pitchDelta = 0;

	// if moving the character in third person, just turn immediately, no deadzones.
	if (vrSystem->thirdPersonMovement) return;
...
void iVr::FrameStart(void)
{
	if (hasOculusRift)
	{
		HMDGetOrientation(poseHmdAngles, poseHmdHeadPositionDelta, poseHmdBodyPositionDelta, poseHmdAbsolutePosition, playerHeight, false);
	...
	if (idLib::frameNumber == lastFrame && !vrSystem->renderingSplash) return;
	lastFrame = idLib::frameNumber;
